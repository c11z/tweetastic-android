...
public class MentionsFragment extends TweetListFragment {
    ...
    protected void moarTweets(RequestParams params) {
        ...
        if (isNetworkAvailable() && !asyncClientRunning) {
            TweetasticApp.getRestClient().getMentions(params, handler);
        }
    }

    protected List<Tweet> getAllTweets() {
        return new Select().from(Tweet.class).where("Mention='true'").orderBy("TweetId DESC").execute();
    }

    protected void dealWithNewTweets(ArrayList<Tweet> newTweets) {
        for (int i=0; i < newTweets.size(); i++) {
            Tweet tweet = newTweets.get(i);
            tweet.setMention(true);
            // Save to database
            tweet.getUser().save();
            tweet.save();
